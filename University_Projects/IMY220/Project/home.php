...
if (isset($_POST['loginsbm'])) {
    $email = isset($_POST["loginEmail"]) ? htmlspecialchars($_POST["loginEmail"]) : false;
    $pass = isset($_POST["loginPword"]) ? htmlspecialchars($_POST["loginPword"]) : false;

    $qryTBUsers = "SELECT * FROM tbusers WHERE email LIKE '$email' AND password LIKE '$pass';";
    ...
    $query = "INSERT INTO tbusers (name, surname, email, username, password, profile_photo) VALUES ('$name', '$surname', '$email', '$uName', '$pass', 'unknown.png');";
...
<html lang="en">
                        ...
                        if ($tbGalleryResult->num_rows > 0) {
                            while ($row = $tbGalleryResult->fetch_assoc()) {
                                ...
                                echo "<div class='offset-1 col-block-4 col-md-4 col-lg-3 mt-3' id='" . $row['image_id'] . "'>
                                                <div class='card'>
                                                    <a class='postCard' href='posts.php?imageID=" . $row['image_id'] . "&user=" . $tbusersresult . "'>
                                                        <img class='card-img-top' src='Gallery/" . $row['filename'] . "' alt='Image' />
                                                    </a>
                                                        
                                                    <div class='card-body'>
                                                        <div class='card-title'>
                                                            <h4><a href='user.php?request="."home"."&visit=".$row['user_id']."'> $tbusersresult </a></h4>
                                                        </div>
                                                        <div class='card-text'>
                                                            <p>" . $row['caption'] . "</p>
                                                            <hr />
    ...
    echo "<div class = 'container'>
                        ...
                        <div class='row mt-5'>
                            <p id='userNotInDataBaseText'>
                                Oh No! It looks like you aren't registered on this site!
                                <br />
                                <b><a href='index.php'>Go back to sign-up page?</a></b>
                            </p>
                        </div>
                        
                        <img id='notUser' src='images/notUser.png' alt='notUser.png'/>
                    </div>";
}
?>
